...
struct GenericOverallCurrentState : public Structs::OverallCurrentStateStruct::Type
{
    ...
    // 为 std::optional 添加自由的 operator==（修复 GCC 14.2 编译错误）
    friend bool operator==(const GenericOverallCurrentState & lhs, const GenericOverallCurrentState & rhs)
    {
        return lhs.position == rhs.position && lhs.latch == rhs.latch && lhs.speed == rhs.speed &&
               lhs.secureState == rhs.secureState;
    }
};
...
struct GenericOverallTargetState : public Structs::OverallTargetStateStruct::Type
{
    ...
    // 为 std::optional 添加自由的 operator==（修复 GCC 14.2 编译错误）
    friend bool operator==(const GenericOverallTargetState & lhs, const GenericOverallTargetState & rhs)
    {
        return lhs.position == rhs.position && lhs.latch == rhs.latch && lhs.speed == rhs.speed;
    }
};
