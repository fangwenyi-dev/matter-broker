...
CHIP_ERROR EspDnssdPublishService(const DnssdService * service, DnssdPublishCallback callback, void * context)
{
    ...
    // =====================================================================
    // [matter-broker 重要补丁] 禁止取消以下注释！
    // =====================================================================
    // 原因：
    // 1. main.cpp 中已通过 mdns_hostname_set() 将 hostname 设为 "matter-broker"
    //    用于 LoRa 网关通过 mDNS 解析 broker 地址（matter-broker.local）
    // 2. 若取消此处注释，Matter SDK 会将 hostname 覆盖为 MAC hex（如 123456789abc.local）
    //    导致 LoRa 网关无法解析 matter-broker.local，MQTT 连接失败
    // 3. Matter 服务的 SRV/TXT 记录仍会正确发布（使用全局 hostname "matter-broker"）
    //    不影响 Matter 设备发现和配网
    // =====================================================================
    // 若需恢复默认行为，请确保有替代方案保证 LoRa 网关能解析到 broker 地址
    // =====================================================================
    ...
    {
        // =================================================================
        // [matter-broker 重要补丁] 第4个参数传 NULL，禁止改为 service->mHostName
        // =================================================================
        // 原因：
        // 1. 全局 hostname 已在 main.cpp 中设为 "matter-broker"
        // 2. service->mHostName 是 MAC hex（如 123456789abc），对应的 hostname 不存在
        // 3. 若传 service->mHostName，mdns_service_subtype_add_for_host 会
        //    查找该 hostname 的服务，找不到就失败，导致 subtype 添加失败
        // 4. 传 NULL 表示使用全局 hostname（matter-broker），能正确添加 subtype
        // =================================================================
        mdns_service_subtype_add_for_host(service->mName, service->mType, GetProtocolString(service->mProtocol), NULL,
                                          service->mSubTypes[i]);
    }
#endif
    VerifyOrExit(espError == ESP_OK, error = CHIP_ERROR_INTERNAL);
